...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS17")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS17")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS17")) || [];
...
if(parseInt(localStorage.getItem("SumaDS17"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS17",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS17"); 
    localStorage.removeItem("contadorDS17"); 
    window.location.reload();
    localStorage.setItem("contadorDS17",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS17"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS17"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS17",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS17")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS17",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS17"))==1){
            let con = parseInt(localStorage.getItem("contadorDS17")) 
            let total= con + precio
            localStorage.setItem("contadorDS17",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS17", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS17", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS17", JSON.stringify(Cuenta));
        mostar();
