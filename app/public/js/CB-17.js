...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-CB17")) || []
let contador = JSON.parse(localStorage.getItem("contador-CB17")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-CB17")) || [];
...
if(parseInt(localStorage.getItem("Suma-CB17"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-CB17",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-CB17"); 
    localStorage.removeItem("contador-CB17"); 
    window.location.reload();
    localStorage.setItem("contador-CB17",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-CB17"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-CB17"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-CB17",sumatoria)
        ini=1
        localStorage.removeItem("Suma-CB17")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-CB17",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-CB17"))==1){
            let con = parseInt(localStorage.getItem("contador-CB17")) 
            let total= con + precio
            localStorage.setItem("contador-CB17",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-CB17", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-CB17", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-CB17", JSON.stringify(Cuenta));
        mostar();
