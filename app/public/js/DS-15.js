...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS15")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS15")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS15")) || [];
...
if(parseInt(localStorage.getItem("SumaDS15"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS15",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS15"); 
    localStorage.removeItem("contadorDS15"); 
    window.location.reload();
    localStorage.setItem("contadorDS15",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS15"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS15"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS15",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS15")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS15",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS15"))==1){
            let con = parseInt(localStorage.getItem("contadorDS15")) 
            let total= con + precio
            localStorage.setItem("contadorDS15",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS15", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS15", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS15", JSON.stringify(Cuenta));
        mostar();
