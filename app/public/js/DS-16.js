...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS16")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS16")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS16")) || [];
...
if(parseInt(localStorage.getItem("SumaDS16"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS16",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS16"); 
    localStorage.removeItem("contadorDS16"); 
    window.location.reload();
    localStorage.setItem("contadorDS16",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS16"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS16"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS16",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS16")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS16",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS16"))==1){
            let con = parseInt(localStorage.getItem("contadorDS16")) 
            let total= con + precio
            localStorage.setItem("contadorDS16",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS16", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS16", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS16", JSON.stringify(Cuenta));
        mostar();
