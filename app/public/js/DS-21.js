...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS21")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS21")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS21")) || [];
...
if(parseInt(localStorage.getItem("SumaDS21"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS21",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS21"); 
    localStorage.removeItem("contadorDS21"); 
    window.location.reload();
    localStorage.setItem("contadorDS21",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS21"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS21"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS21",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS21")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS21",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS21"))==1){
            let con = parseInt(localStorage.getItem("contadorDS21")) 
            let total= con + precio
            localStorage.setItem("contadorDS21",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS21", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS21", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS21", JSON.stringify(Cuenta));
        mostar();
