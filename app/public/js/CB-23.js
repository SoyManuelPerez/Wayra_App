...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-CB23")) || []
let contador = JSON.parse(localStorage.getItem("contador-CB23")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-CB23")) || [];
...
if(parseInt(localStorage.getItem("Suma-CB23"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-CB23",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-CB23"); 
    localStorage.removeItem("contador-CB23"); 
    window.location.reload();
    localStorage.setItem("contador-CB23",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-CB23"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-CB23"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-CB23",sumatoria)
        ini=1
        localStorage.removeItem("Suma-CB23")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-CB23",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-CB23"))==1){
            let con = parseInt(localStorage.getItem("contador-CB23")) 
            let total= con + precio
            localStorage.setItem("contador-CB23",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-CB23", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-CB23", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-CB23", JSON.stringify(Cuenta));
        mostar();
