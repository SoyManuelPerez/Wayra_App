...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS12")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS12")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS12")) || [];
...
if(parseInt(localStorage.getItem("SumaDS12"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS12",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS12"); 
    localStorage.removeItem("contadorDS12"); 
    window.location.reload();
    localStorage.setItem("contadorDS12",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS12"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS12"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS12",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS12")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS12",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS12"))==1){
            let con = parseInt(localStorage.getItem("contadorDS12")) 
            let total= con + precio
            localStorage.setItem("contadorDS12",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS12", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS12", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS12", JSON.stringify(Cuenta));
        mostar();
