...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-HB3")) || []
let contador = JSON.parse(localStorage.getItem("contador-HB3")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-HB3")) || [];
...
if(parseInt(localStorage.getItem("Suma-HB3"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-HB3",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-HB3"); 
    localStorage.removeItem("contador-HB3"); 
    window.location.reload();
    localStorage.setItem("contador-HB3",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-HB3"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-HB3"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-HB3",sumatoria)
        ini=1
        localStorage.removeItem("Suma-HB3")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-HB3",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-HB3"))==1){
            let con = parseInt(localStorage.getItem("contador-HB3")) 
            let total= con + precio
            localStorage.setItem("contador-HB3",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-HB3", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-HB3", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-HB3", JSON.stringify(Cuenta));
        mostar();
