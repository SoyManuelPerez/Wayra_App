...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-CB22")) || []
let contador = JSON.parse(localStorage.getItem("contador-CB22")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-CB22")) || [];
...
if(parseInt(localStorage.getItem("Suma-CB22"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-CB22",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-CB22"); 
    localStorage.removeItem("contador-CB22"); 
    window.location.reload();
    localStorage.setItem("contador-CB22",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-CB22"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-CB22"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-CB22",sumatoria)
        ini=1
        localStorage.removeItem("Suma-CB22")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-CB22",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-CB22"))==1){
            let con = parseInt(localStorage.getItem("contador-CB22")) 
            let total= con + precio
            localStorage.setItem("contador-CB22",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-CB22", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-CB22", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-CB22", JSON.stringify(Cuenta));
        mostar();
