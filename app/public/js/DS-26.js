...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS26")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS26")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS26")) || [];
...
if(parseInt(localStorage.getItem("SumaDS26"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS26",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS26"); 
    localStorage.removeItem("contadorDS26"); 
    window.location.reload();
    localStorage.setItem("contadorDS26",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS26"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS26"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS26",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS26")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS26",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS26"))==1){
            let con = parseInt(localStorage.getItem("contadorDS26")) 
            let total= con + precio
            localStorage.setItem("contadorDS26",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS26", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS26", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS26", JSON.stringify(Cuenta));
        mostar();
