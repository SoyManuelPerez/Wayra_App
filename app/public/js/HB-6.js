...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-HB6")) || []
let contador = JSON.parse(localStorage.getItem("contador-HB6")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-HB6")) || [];
...
if(parseInt(localStorage.getItem("Suma-HB6"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-HB6",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-HB6"); 
    localStorage.removeItem("contador-HB6"); 
    window.location.reload();
    localStorage.setItem("contador-HB6",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-HB6"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-HB6"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-HB6",sumatoria)
        ini=1
        localStorage.removeItem("Suma-HB6")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-HB6",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-HB6"))==1){
            let con = parseInt(localStorage.getItem("contador-HB6")) 
            let total= con + precio
            localStorage.setItem("contador-HB6",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-HB6", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-HB6", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-HB6", JSON.stringify(Cuenta));
        mostar();
