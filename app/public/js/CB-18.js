...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-CB18")) || []
let contador = JSON.parse(localStorage.getItem("contador-CB18")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-CB18")) || [];
...
if(parseInt(localStorage.getItem("Suma-CB18"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-CB18",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-CB18"); 
    localStorage.removeItem("contador-CB18"); 
    window.location.reload();
    localStorage.setItem("contador-CB18",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-CB18"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-CB18"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-CB18",sumatoria)
        ini=1
        localStorage.removeItem("Suma-CB18")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-CB18",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-CB18"))==1){
            let con = parseInt(localStorage.getItem("contador-CB18")) 
            let total= con + precio
            localStorage.setItem("contador-CB18",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-CB18", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-CB18", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-CB18", JSON.stringify(Cuenta));
        mostar();
