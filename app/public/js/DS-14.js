...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS14")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS14")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS14")) || [];
...
if(parseInt(localStorage.getItem("SumaDS14"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS14",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS14"); 
    localStorage.removeItem("contadorDS14"); 
    window.location.reload();
    localStorage.setItem("contadorDS14",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS14"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS14"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS14",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS14")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS14",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS14"))==1){
            let con = parseInt(localStorage.getItem("contadorDS14")) 
            let total= con + precio
            localStorage.setItem("contadorDS14",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS14", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS14", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS14", JSON.stringify(Cuenta));
        mostar();
