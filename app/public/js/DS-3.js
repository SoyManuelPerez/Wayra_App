...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS3")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS3")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS3")) || [];
...
if(parseInt(localStorage.getItem("SumaDS3"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS3",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS3"); 
    localStorage.removeItem("contadorDS3"); 
    window.location.reload();
    localStorage.setItem("contadorDS3",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS3"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS3"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS3",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS3")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS3",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS3"))==1){
            let con = parseInt(localStorage.getItem("contadorDS3")) 
            let total= con + precio
            localStorage.setItem("contadorDS3",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS3", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS3", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS3", JSON.stringify(Cuenta));
        mostar();
