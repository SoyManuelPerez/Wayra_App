...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS2")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS2")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS2")) || [];
...
if(parseInt(localStorage.getItem("SumaDS2"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS2",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS2"); 
    localStorage.removeItem("contadorDS2"); 
    window.location.reload();
    localStorage.setItem("contadorDS2",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS2"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS2"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS2",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS2")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS2",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS2"))==1){
            let con = parseInt(localStorage.getItem("contadorDS2")) 
            let total= con + precio
            localStorage.setItem("contadorDS2",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS2", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS2", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS2", JSON.stringify(Cuenta));
        mostar();
