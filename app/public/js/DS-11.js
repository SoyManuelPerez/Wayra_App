...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS11")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS11")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS11")) || [];
...
if(parseInt(localStorage.getItem("SumaDS11"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS11",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS11"); 
    localStorage.removeItem("contadorDS11"); 
    window.location.reload();
    localStorage.setItem("contadorDS11",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS11"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS11"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS11",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS11")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS11",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS11"))==1){
            let con = parseInt(localStorage.getItem("contadorDS11")) 
            let total= con + precio
            localStorage.setItem("contadorDS11",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS11", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS11", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS11", JSON.stringify(Cuenta));
        mostar();
