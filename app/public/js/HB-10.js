...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-HB10")) || []
let contador = JSON.parse(localStorage.getItem("contador-HB10")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-HB10")) || [];
...
if(parseInt(localStorage.getItem("Suma-HB10"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-HB10",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-HB10"); 
    localStorage.removeItem("contador-HB10"); 
    window.location.reload();
    localStorage.setItem("contador-HB10",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-HB10"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-HB10"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-HB10",sumatoria)
        ini=1
        localStorage.removeItem("Suma-HB10")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-HB10",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-HB10"))==1){
            let con = parseInt(localStorage.getItem("contador-HB10")) 
            let total= con + precio
            localStorage.setItem("contador-HB10",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-HB10", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-HB10", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-HB10", JSON.stringify(Cuenta));
        mostar();
