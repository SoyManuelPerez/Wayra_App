...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS13")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS13")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS13")) || [];
...
if(parseInt(localStorage.getItem("SumaDS13"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS13",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS13"); 
    localStorage.removeItem("contadorDS13"); 
    window.location.reload();
    localStorage.setItem("contadorDS13",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS13"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS13"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS13",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS13")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS13",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS13"))==1){
            let con = parseInt(localStorage.getItem("contadorDS13")) 
            let total= con + precio
            localStorage.setItem("contadorDS13",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS13", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS13", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS13", JSON.stringify(Cuenta));
        mostar();
