...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS27")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS27")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS27")) || [];
...
if(parseInt(localStorage.getItem("SumaDS27"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS27",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS27"); 
    localStorage.removeItem("contadorDS27"); 
    window.location.reload();
    localStorage.setItem("contadorDS27",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS27"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS27"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS27",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS27")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS27",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS27"))==1){
            let con = parseInt(localStorage.getItem("contadorDS27")) 
            let total= con + precio
            localStorage.setItem("contadorDS27",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS27", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS27", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS27", JSON.stringify(Cuenta));
        mostar();
