...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS8")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS8")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS8")) || [];
...
if(parseInt(localStorage.getItem("SumaDS8"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS8",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS8"); 
    localStorage.removeItem("contadorDS8"); 
    window.location.reload();
    localStorage.setItem("contadorDS8",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS8"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS8"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS8",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS8")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS8",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS8"))==1){
            let con = parseInt(localStorage.getItem("contadorDS8")) 
            let total= con + precio
            localStorage.setItem("contadorDS8",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS8", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS8", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS8", JSON.stringify(Cuenta));
        mostar();
