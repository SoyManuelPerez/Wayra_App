...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-HB5")) || []
let contador = JSON.parse(localStorage.getItem("contador-HB5")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-HB5")) || [];
...
if(parseInt(localStorage.getItem("Suma-HB5"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-HB5",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-HB5"); 
    localStorage.removeItem("contador-HB5"); 
    window.location.reload();
    localStorage.setItem("contador-HB5",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-HB5"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-HB5"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-HB5",sumatoria)
        ini=1
        localStorage.removeItem("Suma-HB5")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-HB5",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-HB5"))==1){
            let con = parseInt(localStorage.getItem("contador-HB5")) 
            let total= con + precio
            localStorage.setItem("contador-HB5",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-HB5", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-HB5", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-HB5", JSON.stringify(Cuenta));
        mostar();
