...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS4")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS4")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS4")) || [];
...
if(parseInt(localStorage.getItem("SumaDS4"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS4",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS4"); 
    localStorage.removeItem("contadorDS4"); 
    window.location.reload();
    localStorage.setItem("contadorDS4",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS4"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS4"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS4",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS4")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS4",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS4"))==1){
            let con = parseInt(localStorage.getItem("contadorDS4")) 
            let total= con + precio
            localStorage.setItem("contadorDS4",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS4", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS4", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS4", JSON.stringify(Cuenta));
        mostar();
