...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS23")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS23")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS23")) || [];
...
if(parseInt(localStorage.getItem("SumaDS23"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS23",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS23"); 
    localStorage.removeItem("contadorDS23"); 
    window.location.reload();
    localStorage.setItem("contadorDS23",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS23"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS23"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS23",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS23")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS23",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS23"))==1){
            let con = parseInt(localStorage.getItem("contadorDS23")) 
            let total= con + precio
            localStorage.setItem("contadorDS23",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS23", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS23", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS23", JSON.stringify(Cuenta));
        mostar();
