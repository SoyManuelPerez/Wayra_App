...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS29")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS29")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS29")) || [];
...
if(parseInt(localStorage.getItem("SumaDS29"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS29",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS29"); 
    localStorage.removeItem("contadorDS29"); 
    window.location.reload();
    localStorage.setItem("contadorDS29",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS29"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS29"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS29",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS29")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS29",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS29"))==1){
            let con = parseInt(localStorage.getItem("contadorDS29")) 
            let total= con + precio
            localStorage.setItem("contadorDS29",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS29", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS29", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS29", JSON.stringify(Cuenta));
        mostar();
