...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDDS10")) || []
let contador = JSON.parse(localStorage.getItem("contadorDDS10")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDDS10")) || [];
...
if(parseInt(localStorage.getItem("SumaDDS10"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDDS10",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDDS10"); 
    localStorage.removeItem("contadorDDS10"); 
    window.location.reload();
    localStorage.setItem("contadorDDS10",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDDS10"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDDS10"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDDS10",sumatoria)
        ini=1
        localStorage.removeItem("SumaDDS10")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDDS10",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDDS10"))==1){
            let con = parseInt(localStorage.getItem("contadorDDS10")) 
            let total= con + precio
            localStorage.setItem("contadorDDS10",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDDS10", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDDS10", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDDS10", JSON.stringify(Cuenta));
        mostar();
