...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS20")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS20")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS20")) || [];
...
if(parseInt(localStorage.getItem("SumaDS20"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS20",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS20"); 
    localStorage.removeItem("contadorDS20"); 
    window.location.reload();
    localStorage.setItem("contadorDS20",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS20"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS20"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS20",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS20")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS20",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS20"))==1){
            let con = parseInt(localStorage.getItem("contadorDS20")) 
            let total= con + precio
            localStorage.setItem("contadorDS20",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS20", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS20", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS20", JSON.stringify(Cuenta));
        mostar();
