...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-HB4")) || []
let contador = JSON.parse(localStorage.getItem("contador-HB4")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-HB4")) || [];
...
if(parseInt(localStorage.getItem("Suma-HB4"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-HB4",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-HB4"); 
    localStorage.removeItem("contador-HB4"); 
    window.location.reload();
    localStorage.setItem("contador-HB4",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-HB4"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-HB4"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-HB4",sumatoria)
        ini=1
        localStorage.removeItem("Suma-HB4")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-HB4",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-HB4"))==1){
            let con = parseInt(localStorage.getItem("contador-HB4")) 
            let total= con + precio
            localStorage.setItem("contador-HB4",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-HB4", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-HB4", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-HB4", JSON.stringify(Cuenta));
        mostar();
