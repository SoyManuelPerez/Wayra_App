...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS9")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS9")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS9")) || [];
...
if(parseInt(localStorage.getItem("SumaDS9"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS9",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS9"); 
    localStorage.removeItem("contadorDS9"); 
    window.location.reload();
    localStorage.setItem("contadorDS9",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS9"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS9"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS9",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS9")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS9",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS9"))==1){
            let con = parseInt(localStorage.getItem("contadorDS9")) 
            let total= con + precio
            localStorage.setItem("contadorDS9",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS9", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS9", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS9", JSON.stringify(Cuenta));
        mostar();
