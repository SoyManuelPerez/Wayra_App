...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS19")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS19")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS19")) || [];
...
if(parseInt(localStorage.getItem("SumaDS19"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS19",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS19"); 
    localStorage.removeItem("contadorDS19"); 
    window.location.reload();
    localStorage.setItem("contadorDS19",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS19"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS19"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS19",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS19")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS19",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS19"))==1){
            let con = parseInt(localStorage.getItem("contadorDS19")) 
            let total= con + precio
            localStorage.setItem("contadorDS19",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS19", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS19", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS19", JSON.stringify(Cuenta));
        mostar();
