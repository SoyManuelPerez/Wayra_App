...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS5")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS5")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS5")) || [];
...
if(parseInt(localStorage.getItem("SumaDS5"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS5",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS5"); 
    localStorage.removeItem("contadorDS5"); 
    window.location.reload();
    localStorage.setItem("contadorDS5",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS5"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS5"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS5",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS5")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS5",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS5"))==1){
            let con = parseInt(localStorage.getItem("contadorDS5")) 
            let total= con + precio
            localStorage.setItem("contadorDS5",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS5", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS5", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS5", JSON.stringify(Cuenta));
        mostar();
