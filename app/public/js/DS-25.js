...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS25")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS25")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS25")) || [];
...
if(parseInt(localStorage.getItem("SumaDS25"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS25",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS25"); 
    localStorage.removeItem("contadorDS25"); 
    window.location.reload();
    localStorage.setItem("contadorDS25",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS25"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS25"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS25",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS25")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS25",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS25"))==1){
            let con = parseInt(localStorage.getItem("contadorDS25")) 
            let total= con + precio
            localStorage.setItem("contadorDS25",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS25", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS25", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS25", JSON.stringify(Cuenta));
        mostar();
