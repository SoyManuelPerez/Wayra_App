...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDDS7")) || []
let contador = JSON.parse(localStorage.getItem("contadorDDS7")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDDS7")) || [];
...
if(parseInt(localStorage.getItem("SumaDDS7"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDDS7",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDDS7"); 
    localStorage.removeItem("contadorDDS7"); 
    window.location.reload();
    localStorage.setItem("contadorDDS7",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDDS7"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDDS7"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDDS7",sumatoria)
        ini=1
        localStorage.removeItem("SumaDDS7")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDDS7",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDDS7"))==1){
            let con = parseInt(localStorage.getItem("contadorDDS7")) 
            let total= con + precio
            localStorage.setItem("contadorDDS7",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDDS7", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDDS7", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDDS7", JSON.stringify(Cuenta));
        mostar();
