...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS18")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS18")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS18")) || [];
...
if(parseInt(localStorage.getItem("SumaDS18"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS18",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS18"); 
    localStorage.removeItem("contadorDS18"); 
    window.location.reload();
    localStorage.setItem("contadorDS18",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS18"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS18"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS18",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS18")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS18",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS18"))==1){
            let con = parseInt(localStorage.getItem("contadorDS18")) 
            let total= con + precio
            localStorage.setItem("contadorDS18",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS18", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS18", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS18", JSON.stringify(Cuenta));
        mostar();
