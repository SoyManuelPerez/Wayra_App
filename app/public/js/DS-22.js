...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS22")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS22")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS22")) || [];
...
if(parseInt(localStorage.getItem("SumaDS22"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS22",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS22"); 
    localStorage.removeItem("contadorDS22"); 
    window.location.reload();
    localStorage.setItem("contadorDS22",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS22"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS22"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS22",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS22")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS22",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS22"))==1){
            let con = parseInt(localStorage.getItem("contadorDS22")) 
            let total= con + precio
            localStorage.setItem("contadorDS22",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS22", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS22", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS22", JSON.stringify(Cuenta));
        mostar();
