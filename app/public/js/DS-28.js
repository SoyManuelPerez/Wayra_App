...
let Cuenta = JSON.parse(localStorage.getItem("CuentaDS28")) || []
let contador = JSON.parse(localStorage.getItem("contadorDS28")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("SumaDS28")) || [];
...
if(parseInt(localStorage.getItem("SumaDS28"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("SumaDS28",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("CuentaDS28"); 
    localStorage.removeItem("contadorDS28"); 
    window.location.reload();
    localStorage.setItem("contadorDS28",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contadorDS28"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("SumaDS28"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contadorDS28",sumatoria)
        ini=1
        localStorage.removeItem("SumaDS28")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("SumaDS28",sum)
        }
        else if(parseInt(localStorage.getItem("SumaDS28"))==1){
            let con = parseInt(localStorage.getItem("contadorDS28")) 
            let total= con + precio
            localStorage.setItem("contadorDS28",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("CuentaDS28", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("RegistroDS28", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("CuentaDS28", JSON.stringify(Cuenta));
        mostar();
