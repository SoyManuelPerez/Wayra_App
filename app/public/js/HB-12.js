...
let Cuenta = JSON.parse(localStorage.getItem("Cuenta-HB12")) || []
let contador = JSON.parse(localStorage.getItem("contador-HB12")) || [];
let Registro = JSON.parse(localStorage.getItem("Registro")) || [];
let Suma = JSON.parse(localStorage.getItem("Suma-HB12")) || [];
...
if(parseInt(localStorage.getItem("Suma-HB12"))!=1||Cuenta==[]){
    let ini = 0
Suma.push(ini)
localStorage.setItem("Suma-HB12",Suma)}
//Para cuadno se paga la factura elimine toda la cuneta
pagofactura.addEventListener("click", (e) => {
    localStorage.removeItem("Cuenta-HB12"); 
    localStorage.removeItem("contador-HB12"); 
    window.location.reload();
    localStorage.setItem("contador-HB12",0); 
})
...
function mostar3() {
    let total = parseInt(localStorage.getItem("contador-HB12"))
    console.log()
    let html  = "";
        html += `
            <tr>
...
function sumatoria(precio){
    if (parseInt(localStorage.getItem("Suma-HB12"))==0){let sumatoria =[]
        sumatoria.push(precio)
        localStorage.setItem("contador-HB12",sumatoria)
        ini=1
        localStorage.removeItem("Suma-HB12")
        let sum=[]
        sum.push(ini)
        localStorage.setItem("Suma-HB12",sum)
        }
        else if(parseInt(localStorage.getItem("Suma-HB12"))==1){
            let con = parseInt(localStorage.getItem("contador-HB12")) 
            let total= con + precio
            localStorage.setItem("contador-HB12",total)
        }
}
...
contenido.addEventListener("click", (e) => {
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Cuenta-HB12", JSON.stringify(Cuenta));
        mostar();
        mostar2()
        mostar3()
    }
...
contenidoC.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("btn-success")) {
        ...
        localStorage.setItem("Registro-HB12", JSON.stringify(Registro));
        const newArray = Cuenta.filter((Cuenta) => Cuenta.id !== id);
        Cuenta = newArray;
        localStorage.setItem("Cuenta-HB12", JSON.stringify(Cuenta));
        mostar();
